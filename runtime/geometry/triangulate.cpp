...
void PolylineDouglasPeuckerRecursive(
    ...
    const std::vector<diagrammar::Vector2f>& in, float tol,
    std::vector<bool>* out) {
  if (bg == ed - 1) return;
  double maxDist = 0;
  size_t mid = bg + 1;
...
std::vector<diagrammar::Vector2f> PolylineDouglasPeuckerIterative(
    const std::vector<diagrammar::Vector2f>& in, float mTol) {
  if (in.size() <= 2) {
    return in;
  }
...
int CounterClockwise(const diagrammar::Vector2f& a, const diagrammar::Vector2f& b, const diagrammar::Vector2f& c) {
...
// checking for collinear/degenerate case
int PointInCircumcenter(const diagrammar::Vector2f& a, const diagrammar::Vector2f& b, const diagrammar::Vector2f& c,
...
std::vector<Triangle2D> InflateAndTriangulate(
  ...
  // one out path
  // for now, we do not allow the holes in the inflated path
...
// this execution is expensive, we should not call it frequently
